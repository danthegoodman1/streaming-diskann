...
| --- | --- | --- | --- |
| Incomplete | Work | 1A: Cargo workspace conversion, core crate untouched | Missing: workspace Cargo.toml + green `cargo publish --dry-run -p streaming-diskann`. |
| Incomplete | Work | 1B: napi-rs scaffold per tinysandbox conventions | Missing: `streaming-diskann-node/` building a loadable `.node` on darwin-arm64. |
| Incomplete | Work | 1C: `Index.create("memory:", config)` + bulkBuild/search/insert/delete/close | Missing: implementation + passing round-trip test. |
| Incomplete | Work | 1D: Float32Array/bigint marshaling | Missing: u128-safe bigint IDs in/out; unsafe `number` rejected with clear error. |
| Incomplete | Test | 1E: vitest wired with smoke suite | Missing: `npm test` running build + vitest green. |
| Incomplete | Gate | Workspace gates + node build green | Missing: fmt/clippy/test/doc/publish-dry-run + `npm test` evidence. |
